...
namespace NT.UI.MVC.Areas.Identity.Pages.Account
{
    public class LoginWith2FaModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            var result = await _signInManager.TwoFactorSignInAsync("Email",authenticatorCode, false, Input.RememberMachine);
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("User with ID '{UserId}' logged in with 2fa.", user.Id);
                    ...
                    if (roles.Any())
                    {
                        ...
                        {
                            redirectUrl = Url.Content("~/Attendant/AttendantFlowOverview");
                        }
                        else
                        {
